...
export const ExportCSV = ({ csvData, titleButton, fileNameStart }) => {
  ...
  const exportToCSV = (csvData) => {
    ...
    const fileStart = fileNameStart
      ? fileNameStart
      : newCSVData[0]["Tên Nhân Viên"];
    const fileName =
      "[" +
      fileStart +
      "][" +
      ...
      newCSVData[0]["Năm"] +
      "-" +
      newCSVData[newCSVData.length - 1]["Ngày"] +
      "/" +
      newCSVData[newCSVData.length - 1]["Tháng"] +
      "/" +
      newCSVData[newCSVData.length - 1]["Năm"] +
      "][" +
      moment() +
      "]";
  ...
  return (
    <Button variant="contained" onClick={(e) => exportToCSV(csvData)}>
      {titleButton}
    </Button>
  );
};
